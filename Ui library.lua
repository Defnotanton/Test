...
function UiLib:CreateWindow(Tittle)
	local Ayyyyyyyyyyyyyy = Instance.new("ScreenGui")
	...
	local Settings = {
		...
		TabsColor = Color3.fromRGB(30,30,30),
	...
	Ayyyyyyyyyyyyyy.Name = "SDAFCDRGTFRGFTRHGTRHGT"
	Ayyyyyyyyyyyyyy.Parent = game.CoreGui
	Ayyyyyyyyyyyyyy.ZIndexBehavior = Enum.ZIndexBehavior.Global

	Main.Name = "Main"
	Main.Parent = Ayyyyyyyyyyyyyy
	...
	Buttonsa.Name = "Buttons"
	Buttonsa.Parent = Tabs
	...
	function e:Theme(Color, Type)
		if Type == "TopBar" then
			...
			Settings.TopBarColor = Color
		elseif Type == "Tabs" then
			Tabs.BackgroundColor3 = Color
			Settings.TabsColor = Color
		...
		elseif Type == "Reset" then
			...
			Settings.TopBarColor = Color3.fromRGB(35,35,35)
			Settings.TabsColor = Color3.fromRGB(30,30,30)
			Settings.BackGroundColor = Color3.fromRGB(25,25,25)
			Settings.ShadowColor = Color3.fromRGB(0,0,0)
			Settings.TextColor = Color3.fromRGB(255,255,255)

			print(Settings.TopBarColor)
		...
		elseif Type == "Save" then
			writefile("Anton's Projects settings.json",  game:GetService("HttpService"):JSONEncode(Settings))
			print(Settings.TopBarColor)
		elseif Type == "Load" then
			local Json =  game:GetService("HttpService"):JSONDecode(readfile("Anton's Projects settings.json"))

			for Index, Object in pairs(Ayyyyyyyyyyyyyy:GetDescendants()) do
				if Object:IsA("TextButton") or Object:IsA("TextLabel") then
					Object.TextColor3 = Json.TextColor
				end
			end

			Main.BackgroundColor3 = Json.BackGroundColor
			Tabs.BackgroundColor3 = Json.TabsColor
			TopBar.BackgroundColor3 = Json.TopBarColor
			Shadow.ImageColor3 = Json.ShadowColor
		end
	end
	...
	function e:Notify(Text2, Tittle2, Function)
		...
		Notifications.Parent = NotificationsList
		...
		UICorner_2.Parent = Notifications

		Line.Name = "Line"
		Line.Parent = Notifications
	...
	NotificationsList.Parent = Ayyyyyyyyyyyyyy
	...
	UIListLayout420.Parent = NotificationsList
	...
	function e:CreatePage(Name)
		...
		function ae:NewDropDown(OptionsTable, Tittle, Function)
			...
			for Index, Optionsa in pairs(OptionsTable) do
				local Button = Instance.new("TextButton")
				local UICorner = Instance.new("UICorner")
		...
		function ae:NewKeyBind(Tittle, Selected, Function)
			...
			UIS.InputBegan:connect(function(keyCode, a)			
				if On and not a then
					TextButton.Text = keyCode.KeyCode.Name
